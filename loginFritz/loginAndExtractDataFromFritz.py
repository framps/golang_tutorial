...
def loginToServer(server,password,port=80):
    ...
    if response.status != 200:
        ...
        if sid == "0000000000000000":
            ...
            print "Challenge + pwd decode: %s" %         ((challenge + '-' + password).decode('iso-8859-1'))
            print "Challenge + pwd decode encoded: %s" % ((challenge + '-' + password).decode('iso-8859-1').encode('utf-16le'))
            challenge_bf = (challenge + '-' + password).decode('iso-8859-1').encode('utf-16le')
            print "Challenge_bf: %s" % (challenge_bf)
            m = hashlib.md5()
            m.update(challenge_bf)
            print "M: %s" % (m.hexdigest().lower())
